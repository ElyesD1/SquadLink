...
import { offlineStorage, CACHE_KEYS } from '@/lib/offlineStorage';
...
export default function PartiesPage() {
  ...
  const fetchCurrentUser = async () => {
    ...
    if (isOffline) {
      const cachedUser = offlineStorage.get<any>(CACHE_KEYS.USER_PROFILE);
      if (cachedUser) {
        setCurrentUserId(cachedUser._id);
        setHasLolAccount(!!cachedUser.lolAccount);
        setHasDiscordAccount(!!cachedUser.discordId);
        console.log('👤 Profil utilisateur chargé depuis le cache');
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Cache user profile for offline use (expires in 24 hours)
        offlineStorage.set(CACHE_KEYS.USER_PROFILE, data, 24 * 60 * 60 * 1000);
  ...
  const fetchParties = async (userId?: string) => {
    try {
      ...
      if (isOffline) {
        ...
        const cachedParties = offlineStorage.get<Party[]>(CACHE_KEYS.PARTIES);
        if (cachedParties) {
          setParties(cachedParties);
          console.log('✅ Données chargées depuis le cache:', cachedParties.length, 'parties');
        } else {
          setParties([]);
          console.log('⚠️ Aucune donnée en cache');
        }
        setIsLoading(false);
        return;
      }
      ...
      if (response.ok) {
        ...
        // Cache the parties data for offline use (expires in 1 hour)
        if (Array.isArray(partiesData)) {
          offlineStorage.set(CACHE_KEYS.PARTIES, partiesData, 60 * 60 * 1000);
          console.log('💾 Données mises en cache pour le mode hors ligne');
        }
      ...
      if (cachedParties) {
        setParties(cachedParties);
        console.log('⚠️ Erreur réseau: données chargées depuis le cache');
      }
    } finally {
      setIsLoading(false);
    }
  ...
  className={`w-full border border-white/10 shadow-md shadow-purple-500/5 backdrop-blur-sm bg-gray-900 text-white hover:shadow-purple-500/10 hover:border-purple-500/20 rounded-xl p-4 focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-300 appearance-none pr-10`}
>
  <option value="" className="bg-gray-900 text-white">All Game Modes</option>
  <option value="ranked_solo_duo" className="bg-gray-900 text-white">Ranked Solo/Duo</option>
  <option value="ranked_flex" className="bg-gray-900 text-white">Ranked Flex</option>
  <option value="aram" className="bg-gray-900 text-white">ARAM</option>
  <option value="draft_pick" className="bg-gray-900 text-white">Draft Pick</option>
</select>
